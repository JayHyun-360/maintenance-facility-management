...
CREATE OR REPLACE FUNCTION wait_for_profile_sync(p_user_id UUID, p_max_attempts INTEGER DEFAULT 10)
RETURNS TABLE(
    synced BOOLEAN,
    attempts INTEGER,
    user_id UUID
...
BEGIN
    WHILE current_attempts < p_max_attempts AND NOT profile_exists LOOP
